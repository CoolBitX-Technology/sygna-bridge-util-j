...
public class ECDSA {
	...
	public static String sign(String message, String privateKey) throws Exception {
		...
		PrivateKey privKey = ECIES.getPrivateKey(parseHexBinary(privateKey));
		sign.initSign(privKey, new SecureRandom());
		byte[] inputData;
		inputData = message.getBytes(StandardCharsets.UTF_8);
		sign.update(inputData);
		byte[] derSignature = sign.sign();
		return getRawSignature(derSignature);
	}
	
	/**
	 * Verify Message({@link StandardCharsets#UTF_8}) with signature and publicKey.
	 * 
	 * @param message
	 * @param signature
	 * @param publicKey
